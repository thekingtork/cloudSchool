...
class AdminController extends Controller
{
    ...
    public function ma11Action()
    {
        ...
        $sede=$em->getRepository('cloudBundle:Sede')->find(1);
        if(!$sede){
    ...
    */
    ...
    public function formrangoUpdateAction(Request $request)
    {
     ...
     public function GestionSedeAction(){
        ...
        $em=$this->getDoctrine()->getEntityManager();
        $sede=$em->getRepository('cloudBundle:Sede')->find(2);
        if($request->getMethod()=='POST'){
            $numero=$request->request->get('numsede');
            return $this->render('cloudBundle:Admin:ma112.html.twig', array('numero'=>$numero,'sedes'=>$sede,'mje'=>''));
        }
        if(!$sede){
